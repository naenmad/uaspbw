<?php
// Edit Order Page with Authentication
session_start();
...
// Get order data if ID is provided
if ($order_id) {
    try {
        // Get order details - handle both numeric ID and order_number
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'update_order' && $order) {
        try {
            $pdo->beginTransaction();

            // Use actual order ID from the fetched order data
            $actual_order_id = $order['id'];

            // Update order
            $stmt = $pdo->prepare("UPDATE orders SET customer_id = ?, order_date = ?, status = ?, notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
            $stmt->execute([
                $_POST['customer_id'],
                $_POST['order_date'],
                $_POST['status'],
            ...
            $stmt->execute([$actual_order_id]);            // Insert updated order items
            $total_amount = 0;
            if (isset($_POST['product_name'])) {
                $stmt = $pdo->prepare("INSERT INTO order_items (order_id, product_id, product_name, quantity, unit_price, total_price) VALUES (?, NULL, ?, ?, ?, ?)");
            ...
            $stmt->execute([$total_amount, $actual_order_id]);

            $pdo->commit();
            $success = "Order berhasil diperbarui!";

            // Refresh order data using actual order ID
            $stmt = $pdo->prepare("SELECT o.*, c.name as customer_name FROM orders o 
                                  LEFT JOIN customers c ON o.customer_id = c.id 
                                  WHERE o.id = ?");
            $stmt->execute([$actual_order_id]);
...
// If no order ID or order not found, redirect to orders page
if (!$order_id || !$order) {
    if (!$error) {
...
<head>
    ...
    <title>Edit Order - Sistem Pencatatan Order</title>
...
<body>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h4 class="card-title mb-1">
                                        ...
                                        Edit Order <?php echo $order ? '#' . htmlspecialchars($order_id) : ''; ?>
                                    </h4>
                                    <p class="text-muted mb-0">Ubah data order yang sudah ada</p>
                                </div>
                                <div>
                                    ...
                                    <?php if ($order): ?>
                                        <a href="order-detail.php?id=<?php echo $order_id; ?>" class="btn btn-info">
                                ...
                                <form method="POST" id="editOrderForm">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea name="notes" id="notes" class="form-control" rows="3"
                                            placeholder="Tambahkan catatan untuk order ini..."><?= htmlspecialchars($order['notes'] ?? '') ?></textarea>
                                    </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="card bg-light">
                                                <div class="card-body">
                                                    ...
                                                    <h4 class="text-primary" id="grand-total">Rp
    ...
    <script>
        let itemCount = <?= count($order_items) ?: 1 ?>;
